...
import com.ruoyi.system.domain.BaseIccard;
...
public class BaseIccardController extends BaseController
{
    ...
    @PostMapping("/export")
    public void export(HttpServletResponse response, BaseIccard baseIccard)
    {
        List<BaseIccard> list = baseIccardService.selectBaseIccardList(baseIccard);
        ExcelUtil<BaseIccard> util = new ExcelUtil<BaseIccard>(BaseIccard.class);
     ...
     * 获取【请填写功能名称】详细信息
     */
    @PreAuthorize("@ss.hasPermi('system:iccard:query')")
    @GetMapping(value = "/{id}")
    ...
    {
        return toAjax(baseIccardService.insertBaseIccard(baseIccard));
    }
    ...
    {
        return toAjax(baseIccardService.updateBaseIccard(baseIccard));
    }
